...
public:
	FORCEINLINE Sphere() {}
	FORCEINLINE Sphere(const Vector3D& center, float radius) :
		data(center.ToVector(radius)) {}
	Sphere(Vector3D* points, uint32 amt);
...
FORCEINLINE Sphere Sphere::moveTo(const Vector3D& destination) const
{
...
FORCEINLINE Vector3D Sphere::getCenter() const
{
	return Vector3D(data);
}

FORCEINLINE bool Sphere::contains(const Vector3D& point, float errorMargin) const
{
	float r = data[3]+errorMargin;
	return point.DistSquared(Vector3D(data)) < r*r;
}
...
FORCEINLINE Sphere Sphere::translate(const Vector3D& amt) const
{
...
FORCEINLINE Sphere Sphere::addPoint(const Vector3D& other) const
{
